...
const FavoriteNotes: React.FC<FavoritesPageProps> = ({ loggedInUser }) => {
  ...
  useEffect(() => {
    async function loadFavorites() {
      try {
        const fetched = await NotesApi.fetchFavoriteNotes();
        setNotes(fetched);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const handleToggleFavorite = async (noteId: string) => {
    try {
      ...
      if (loggedInUser && !isNoteFavorited(updated, loggedInUser._id)) {
  ...
  return (
    <AppLayout>
      <div>
        <Head>
          <title>Favorites | ThyncSpace</title>
        </Head>
        ...
        >
          <div>
            ...
            >
              ...
              {filteredNotes.length === 1 ? "favorite" : "favorites"}
            </p>
          </div>
